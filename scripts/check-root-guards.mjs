 ...
 * 一旦它们从编排器表里消失，本脚本会红 —— 那种时候必须有人显式决定"docs-only 的 PR
 * 还需要跑什么"，而不是让这条路径悄悄变空。
 *
 * ## 用法与退出码
 *
 * 用法：node scripts/check-root-guards.mjs [--list] [--concurrency N] [--full-output]
 * 退出码：0 = 守卫全部通过（advisory 除外）；1 = 有守卫失败；2 = 用法/解析错误。
 */
...
function parseArgs(argv) {
  const options = { list: false, concurrency: null, fullOutput: false }
...
const missingRequired = MINIMUM_REQUIRED_GUARDS.filter(name => !guards.some(guard => guard.name === name))
if (missingRequired.length > 0) {
  console.error(`check-root-guards: 编排器表里缺少下限要求的守卫：${missingRequired.join(', ')}`)
  console.error('  ⇒ 这些守卫的判据覆盖文档/提交信息，docs-only 的 PR 必须跑到它们。'
...
const failed = results.filter(result => !result.ok && !result.guard.advisory)
const advisory = results.filter(result => !result.ok && result.guard.advisory)
console.log(`──── ${results.length} 个根守卫：${results.length - failed.length - advisory.length} 通过、`
  + `${failed.length} 失败、${advisory.length} 告警(advisory)，总耗时 ${((Date.now() - startedAt) / 1000).toFixed(1)}s`)

for (const result of [...failed, ...advisory]) {
  console.error(`\n===== ${result.guard.name} ${result.guard.advisory ? '失败(advisory，不拦门禁)' : '失败'} =====`)
