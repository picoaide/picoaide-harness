...
export const zh = {
  ...
  'modelsTab.guide.config.item5': '图片输入能力：模型显式声明支持图片输入时显示「🖼 图片输入」标记（来自 DSH 模型能力元数据 inputModalities，只读展示）；未声明=未知，不显示。',
  ...
  'panel.guide.uiSettings.desc': '给 DSH web 界面加样式级小功能（纯客户端注入，不改框架）：各功能的独立小开关在「Web UI 设置」Tab 的「综合」里——会话筛选（左侧列表只显示进行中）、对话区加宽（中间区域扩大到约 95%）等；后期扩展主题更换。',
  'panel.guide.canvas.title': '无限画板',
  'panel.guide.canvas.desc': '把散落在各处的文件/图片/音频集中到一块**无限画布**上（对话页「画板」Tab）——搜索/路径一键上板（本地路径引用，不拷贝）、卡片内直接预览图片音视频、「打开」用系统默认应用查看、「保存」把文本便签落盘到本机任意路径；**归属三档**：本会话 / 本项目 / 所有项目可见，卡片「归属」按钮可随时迁移，其他会话的节点可「跳转」直达；**AI 双向拉取**：de_canvas 工具查画板、按 id 读内容、往画板中央区放便签（不注入上下文，需要时主动查）。**开关**：独立「无限画板」（canvasEnabled，默认关）；关闭时画板 Tab 与工具完全不可见（数据文件保留）。',
...
export const en: Record<MemoryEvolveKey, string> = {
  ...
  'modelsTab.guide.config.item5': 'Image input: models that explicitly declare image support show a "🖼 Image input" badge (from DSH model capability metadata inputModalities, read-only); absent = unknown, no badge.',
  ...
  'modelsTab.guide.switch.body': 'Model Settings are on by default; they can be turned off independently in the "Memory Evolve Settings" tab ("Config") like other modules — the tab and the de_models tool hide, settings data is kept.',
