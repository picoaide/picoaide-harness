...
感谢你愿意参与 PicoAide Harness。这是一个社区项目，无论你是普通用户、插件作者还是开发者，都有适合你的贡献方式。

## 普通用户：使用、反馈与传播

- 遇到问题或异常，[提 issue](https://github.com/picoaide/picoaide-harness/issues)：说明操作系统（macOS / Windows）、应用版本和复现步骤。
- 有功能想法或改进建议，也欢迎提 issue 讨论。
- 参与[社区交流](README.md#社区交流)，帮助其他用户解决问题。
...
- [DSH Community Fabric Draft](community/fabric/README.zh.md)：参与 Manifest、Capability、Host Descriptor 和事件 contract 的公开讨论。
...
- 桌面代码位于 `packages/host/desktop/`；`community/fabric/` 保存社区标准 Draft，当前只有文档、尚不可加载，与外层 Yarn workspace 一起组合。
...
- 变更生产依赖后，运行 `yarn workspace dsh-plugin-desktop verify:notices` 刷新第三方许可清单，并提交更新后的 `packages/host/desktop/THIRD_PARTY_NOTICES.md`。
