...
审核通过 → GET /api/client/v2/agent-presets 全员可见 → /api/pico/agent-presets/:name/install
...
- **可见性**:员工(GET /api/client/v2/agent-presets)= approved 全部 + 自己上传的全部状态;他人 pending/rejected 与不存在同 404(不泄露)。
- **可见性**:员工(GET /api/client/v2/agent-presets)= 已授权用户的 approved + 自己上传的全部状态;他人 pending/rejected 与不存在同 404(不泄露)。**审核通过后仍需管理员授权给用户/部门组才可见可安装**(与商城授权制一致;admin 恒全量不落授权表);同名多版本共享一个授权。

## 3. 端点

- 员工(Bearer):`GET /api/client/v2/agent-presets`、`POST /api/client/v2/agent-presets`(归档 base64 + display_name/description ≤500 字)、`GET /api/client/v2/agent-presets/:name/archive`(仅 approved)。
...
- 统一聚合(Bearer):`GET /api/client/v2/capabilities?source=org`(与共享技能合并返回,见 06-database.md §capabilities)。
