...
- **上传模式(0040)**:管理端「上传新版」提交 `.zip`,归档直接存 `app_releases.archive`(DB 行;0053 统一应用模型前是 `skills.archive`,该表已随 0055 DROP);下载不再走磁盘 clone/打包。
...
- **下载/调用统计(0040)**:`GET /archive` 成功即 `downloads+1`;客户端 `POST /api/client/v2/telemetry/skill-call` 上报技能调用(模型 `skill` 工具成功 / 用户 `/name` 手势注入),服务端按 `app_releases(kind='skill', app_id, version)` 累加 `calls`(0053 前分别是 shared_skills / 市场 skills 两张表)。
