...
// TestSyncDirectoryNoUid:目录条目无 uid(如 example-a 系,只有 cn/mail)——
...
func TestSyncDirectoryNoUid(t *testing.T) {
	db := mustDB(t)
	users := []*ldap.Entry{
		dirUser("cn=ranting,ou=users,dc=example-a,dc=com", "ranting", "ranting", "ranting@a.example.com"),
		dirUser("cn=xiedi,ou=users,dc=example-a,dc=com", "xiedi", "xiedi", "xiedi@a.example.com"),
	}
	...
	f := &fakeLDAPConn{
		passwords: map[string]string{"cn=admin,dc=example-a,dc=com": "pw", "cn=ranting,ou=people,dc=example-a,dc=com": "pw"},
		searchResults: map[string]*ldap.SearchResult{
			"(&(cn=*)(objectClass=inetOrgPerson))": {Entries: users},
		},
	}
	p := &LDAPProvider{ServerURL: "ldap://fake", BindDN: "cn=admin,dc=example-a,dc=com", BindPassword: "pw", BaseDN: "dc=example-a,dc=com", UserFilter: "(&(cn=%s)(objectClass=inetOrgPerson))", GroupAttr: "cn"}
...
// TestSyncDirectoryUserAttrCn:user_attr=cn 的目录(如 example-a 系,无 uid)——
	...
	f := &fakeLDAPConn{
		...
		searchResults: map[string]*ldap.SearchResult{
			...
			"(&(cn=*)(objectClass=inetOrgPerson))": {Entries: []*ldap.Entry{
				{ // 无 uid,仅 cn/mail/sn(与生产 example-a 目录一致)
					DN: "cn=ranting,ou=people,dc=example-a,dc=com",
					Attributes: []*ldap.EntryAttribute{
						{Name: "cn", Values: []string{"ranting"}},
						{Name: "sn", Values: []string{"冉婷"}},
						{Name: "mail", Values: []string{"ranting@a.example.com"}},
					},
				},
				{
					DN: "cn=xiedi,ou=people,dc=example-a,dc=com",
					Attributes: []*ldap.EntryAttribute{
						{Name: "cn", Values: []string{"xiedi"}},
						{Name: "sn", Values: []string{"谢迪"}},
						{Name: "mail", Values: []string{"xiedi@a.example.com"}},
					},
				},
			}},
			"(&(cn=ranting)(objectClass=inetOrgPerson))": {Entries: []*ldap.Entry{
				{DN: "cn=ranting,ou=people,dc=example-a,dc=com",
					Attributes: []*ldap.EntryAttribute{
						{Name: "cn", Values: []string{"ranting"}}, {Name: "sn", Values: []string{"冉婷"}}, {Name: "mail", Values: []string{"ranting@a.example.com"}},
					}},
			}},
			"(member=cn=ranting,ou=people,dc=example-a,dc=com)": {Entries: []*ldap.Entry{
				{DN: "cn=SSC,ou=groups,dc=example-a,dc=com", Attributes: []*ldap.EntryAttribute{{Name: "cn", Values: []string{"SSC"}}}},
			}},
		},
	}
	p := &LDAPProvider{
		ServerURL: "ldap://fake", BindDN: "cn=admin,dc=example-a,dc=com", BindPassword: "pw",
		BaseDN: "dc=example-a,dc=com", UserFilter: "(&(cn=%s)(objectClass=inetOrgPerson))",
