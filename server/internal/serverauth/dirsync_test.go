...
func TestSyncDirectoryNoUid(t *testing.T) {
	db := mustDB(t)
	users := []*ldap.Entry{
		dirUser("cn=alice,ou=users,dc=example-a,dc=com", "alice", "alice", "alice@a.example.com"),
		dirUser("cn=bob,ou=users,dc=example-a,dc=com", "bob", "bob", "bob@a.example.com"),
	}
	...
	f := &fakeLDAPConn{
		passwords: map[string]string{"cn=admin,dc=example-a,dc=com": "pw", "cn=alice,ou=people,dc=example-a,dc=com": "pw"},
		searchResults: map[string]*ldap.SearchResult{
		...
		searchResults: map[string]*ldap.SearchResult{
			...
			"(&(cn=*)(objectClass=inetOrgPerson))": {Entries: []*ldap.Entry{
				{ // 无 uid,仅 cn/mail/sn(与生产 example-a 目录一致)
					DN: "cn=alice,ou=people,dc=example-a,dc=com",
					Attributes: []*ldap.EntryAttribute{
						{Name: "cn", Values: []string{"alice"}},
						{Name: "sn", Values: []string{"zhangsan"}},
						{Name: "mail", Values: []string{"alice@a.example.com"}},
					},
				},
				{
					DN: "cn=bob,ou=people,dc=example-a,dc=com",
					Attributes: []*ldap.EntryAttribute{
						{Name: "cn", Values: []string{"bob"}},
						{Name: "sn", Values: []string{"谢迪"}},
						{Name: "mail", Values: []string{"bob@a.example.com"}},
					},
				},
			}},
			"(&(cn=alice)(objectClass=inetOrgPerson))": {Entries: []*ldap.Entry{
				{DN: "cn=alice,ou=people,dc=example-a,dc=com",
					Attributes: []*ldap.EntryAttribute{
						{Name: "cn", Values: []string{"alice"}}, {Name: "sn", Values: []string{"zhangsan"}}, {Name: "mail", Values: []string{"alice@a.example.com"}},
					}},
			}},
			"(member=cn=alice,ou=people,dc=example-a,dc=com)": {Entries: []*ldap.Entry{
