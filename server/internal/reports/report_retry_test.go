...
// 调度器每小时那一轮会重新生成**同一期**（GenerateMonthlyReport 取"上月"，本月内不变）
...
func TestFailedMonthlyReportIsRetriedByNextSchedulerRound(t *testing.T) {
	...
	if !ShouldRunMonthly(now, subs[0].LastRunAt) {
		t.Fatal("失败后 ShouldRunMonthly = false ⇒ 本北京月内零重试（修前的缺陷形态）")
	}

	// ② webhook 恢复 ⇒ 下一轮（同一小时内）必须补投**同一期**（失败的那一期），不是新一期。
	mu.Lock()
	failing = false
	mu.Unlock()
	if err := sched.tryRun(); err != nil {
...
func TestRetrySurvivesAcrossSchedulerRoundsUntilSuccess(t *testing.T) {
	...
	now := bjAt(2026, 9, 20, 10)
	sched := NewScheduler(db, time.Hour, func() time.Time { return now })
	for round := 1; round <= 4; round++ {
		err := sched.tryRun()
		if round <= 3 && err == nil {
			t.Fatalf("第 %d 轮（webhook 仍在 5xx）必须报错", round)
		}
		if round == 4 && err != nil {
