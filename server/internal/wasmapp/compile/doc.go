...
//     Chmod，最后校验），违规在 `IsolationRequire` 下**拒绝启动**；其余档位（**compose 默认
//     就是 auto**）**拒绝启动不是选项，但也不照用**（2026-09-21 四轮审计 P2-①）：
//     wazero 的磁盘缓存是**读 + 写**，子进程命中就直接加载那份"机器码"并**干跑**它，
//     而 `auto` 档在没有 bwrap 的机器上不隔离 ⇒ "能布置缓存目录的人"可以在读得到数据根的
//     编译进程里执行代码。现在改为：不可信 ⇒ 子进程改用一个**本进程新建的 0700 临时目录**
//     （既不读攻击者的条目，也不往那棵树里写），随 `Compiler.Close` 清理；
//     判据 `TestCompilerAvoidsUntrustedCacheDir`（含"攻击者目录必须一个文件都没有"与
//     "可信时仍原样使用配置目录"两条反向对照）。
