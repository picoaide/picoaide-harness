...
// 同名表完全同形、`err=nil`、数字是错的（真 PG 实测：public `2 人 / 150.00` vs
// 敌对池 `3 人 / 6166.00`）。
//
...
//
// 变异验证：把两条聚合读改回 `db.QueryRow(...)`（裸池）⇒ 本用例红（读回 3 人 / 6166.00）。
...
// shadow 侧诱饵的**判据锚点**：人数与合计必须与 public 真值可区分（3 人 / 6166.00），
// 否则主判据会退化成恒真。
//
// 诱饵三人 = `3333.00 + 3333.00 − 500.00`（R14-O · VC-F4 订正：旧注释写 9999.00，
// 而 9999.00 从未是任何一版夹具的值；同一处的"反向自证"断言也拿它比较 ⇒ 恒假断言，
// 永远不会触发。现在改为对**实际诱饵**的断言，且这条断言真的能失败：
// 改错任一诱饵值 ⇒ 观测值 ≠ 常量；把诱饵调成与 public 同值 ⇒ "不可区分"断言红）。
const (
	r14kShadowDecoyUsers = 3
	r14kShadowDecoyTotal = 6166.00
)
...
func r14kShadowPool(t *testing.T, db *sql.DB) *sql.DB {
	...
	// 诱饵：3 个启用普通员工 —— 两人 +3333.00、一人 −500.00（合计 6166.00；public 侧
	// 的真值是 2 人 / 合计 150.00 / 无人欠款 ⇒ 人数、合计、欠款三处都能分辨读的是哪个库）。
	for i := 1; i <= r14kShadowDecoyUsers; i++ {
		balance := 3333.00
		if i == 3 {
			balance = -500.00
		}
...
func TestAuditR14KBalanceSummaryAggregatesReadPublic(t *testing.T) {
	...
	// 反向自证（R14-O · VC-F4 重写）：对**裸池**执行与 GetBalanceSummary 逐字相同的
	// 聚合读，必须读到 shadow 的诱饵值本身。
	//
	// 为什么要这一条：主判据的形态是"got 必须等于 public 真值" —— 如果 shadow 里
	// 根本没有同名表/没有诱饵行（夹具失效），got 会**回落到 public** 而让主判据恒真。
	// 旧版本在这里写的是 `if got.Total == 9999.00`（9999.00 不是任何一版夹具的值）
	// ⇒ 恒假断言、永不触发。现在两条断言都直接锚在**可观测的裸池读数**上，且都能失败：
	//   - 诱饵值被改错/没播上 ⇒ 观测值 ≠ 常量；
	//   - 诱饵被改成与 public 同值 ⇒ "不可区分"断言红（主判据会退化成恒真）。
	var bareUsers int64
	var bareTotal float64
	if err := side.QueryRow(`SELECT COUNT(*), COALESCE(SUM(balance_money),0) FROM users
WHERE status = 1 AND role = ?`, RoleUser).Scan(&bareUsers, &bareTotal); err != nil {
		t.Fatalf("裸池读 shadow（反向自证）: %v", err)
	}
	if bareUsers != r14kShadowDecoyUsers || bareTotal != r14kShadowDecoyTotal {
		t.Fatalf("夹具反向自证失败：裸池读 shadow = %d 人 / %.2f，期望诱饵 %d 人 / %.2f"+
			"（诱饵值被改动或 shadow 未生效）—— 主判据会因此退化成恒真，必须先修夹具",
			bareUsers, bareTotal, r14kShadowDecoyUsers, r14kShadowDecoyTotal)
	}
	if bareUsers == want.Users || bareTotal == want.Total {
		t.Fatalf("夹具反向自证失败：诱饵（%d 人 / %.2f）与 public 真值（%d 人 / %.2f）不可区分"+
			" ⇒ 主判据读哪个库都会通过", bareUsers, bareTotal, want.Users, want.Total)
	}
}
