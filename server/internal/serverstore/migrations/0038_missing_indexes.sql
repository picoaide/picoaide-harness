-- 0038: 缺失索引补全(对齐 PG 0038,基于 EXPLAIN QUERY PLAN 实测)。
-- 审计发现(2026-08-26):
...
--      'group' AND ...) —— 权限热路径(每次请求),现有 PK 是
--      (resource, grantee_type, grantee),grantee 非前缀无法使用 → 全表扫描。
--     需 (grantee_type, grantee) 复合索引。
