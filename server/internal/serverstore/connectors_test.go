...
func TestConnectorCRUD(t *testing.T) {
	...
	// 种子:迁移 0042 插入 example-a + glitchtip。
	...
	if !ids["example-a"] || !ids["glitchtip"] || !ids["sales-easy"] {
		t.Fatalf("seed missing example-a/glitchtip/sales-easy: %v", ids)
	}
...
func TestConnectorDefinitionPolicy(t *testing.T) {
	...
	accepted := []struct {
		...
		// 0042 的三个种子定义必须继续通过。
		{"seed example-a", `{"auth":{"discoveryUrl":"https://mcp-a.example.com/mcp"},"mcp":[{"serverName":"example-a","transport":"streamable-http","url":"https://mcp-a.example.com/mcp"}]}`},
