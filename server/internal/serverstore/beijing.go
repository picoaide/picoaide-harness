...
// 后果：进程恰好在"目标月没有该日"的那几天启动时，保留期内**最早一个月**的日账/月账洞
// 不会被本轮或后续任何清理轮修补 —— 一旦明细后来被回收，报表静默少计那个月。
//
// **不要在这里手写"哪几天会溢出"的日期清单**（V13-B 复审实测：手写清单必错 ——
// 溢出与否取决于 `源日 d`、`回溯月数 N` 与 `目标月长度` 三者的组合：d=31 时目标月为
// 30 天的月份才溢出（如 3/31−6月→9/30）、目标月有 31 天则不溢出（如 7/31−6月→1/31）；
// d=29 只在目标 2 月非闰年时溢出）。**判据语料是唯一的权威清单**：
// `audit_r13ge_calendar_test.go` 按 (年, 月, 日, N) 表驱动枚举（含 2026–2028 与闰年），
// 实现改回 `BeijingDay+AddDate` 时该用例必红。
//
