...
// 因此 bootstrap 只下发 example-a + sales-easy, glitchtip 不在列表中
...
func TestBootstrapConnectors(t *testing.T) {
	...
	if len(conns) < 2 {
		t.Fatalf("connectors = %d, want >= 2 (种子 example-a+sales-easy)", len(conns))
	}
	...
	// example-a / sales-easy 在;glitchtip(0045 下架)不在。
	if byID["example-a"] == nil || byID["sales-easy"] == nil {
	...
	// 禁用 example-a → 不再出现。
	if err := serverstore.SetConnectorEnabled(db, "example-a", false); err != nil {
		t.Fatal(err)
	}
	_, out = getJSON(t, r, "/api/client/v2/config/bootstrap", token)
	conns = out["connectors"].([]any)
	for _, c := range conns {
		m := c.(map[string]any)
		if m["id"] == "example-a" {
			t.Fatalf("disabled example-a still in connectors")
		}
	}
}
