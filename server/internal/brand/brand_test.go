...
func TestBrandEnableAndLogoFlow(t *testing.T) {
	...
	w := doReq(t, r, "PUT", "/api/admin/brand", reqBody, sess, csrf)
	if w.Code != 200 {
	...
	w = doSend(t, r, "POST", "/api/admin/brand/logo", upload, ct, sess, csrf)
	if w.Code != 200 {
		t.Fatalf("upload logo = %d body=%s", w.Code, w.Body.String())
	}
	// 3. 公开 brand 返回 logo_url
	w = doReq(t, r, "GET", "/api/brand", "", "", "")
	if w.Code != 200 {
		t.Fatalf("get brand = %d", w.Code)
	}
	var b map[string]any
	_ = json.Unmarshal(w.Body.Bytes(), &b)
	if b["login"].(map[string]any)["logo_url"] != "/api/brand/logo/login" {
		t.Fatalf("login logo_url missing: %v", b)
	}
	// 4. 下载 logo
	w = doReq(t, r, "GET", "/api/brand/logo/login", "", "", "")
...
func TestUploadLogoSanitizeSVG(t *testing.T) {
	...
	w = doReq(t, r, "GET", "/api/brand/logo/client", "", "", "")
	body := w.Body.String()
...
func TestBrandSnapshotRestore(t *testing.T) {
	...
	sess, csrf := adminSession(t, db)
	// 第一版
	doReq(t, r, "PUT", "/api/admin/brand", `{"enabled":true,"login":{"display_name":"V1"},"client":{},"title":""}`, sess, csrf)
	// 第二版(触发快照存 V1)
	doReq(t, r, "PUT", "/api/admin/brand", `{"enabled":true,"login":{"display_name":"V2"},"client":{},"title":""}`, sess, csrf)
	// 列出快照
	w := doReq(t, r, "GET", "/api/admin/brand/snapshots", "", sess, csrf)
	var out struct {
		Snapshots []struct {
			ID   int64  `json:"id"`
	...
	w = doReq(t, r, "POST", "/api/admin/brand/restore", fmt.Sprintf(`{"id":%d}`, sid), sess, csrf)
	if w.Code != 200 {
		t.Fatalf("restore = %d", w.Code)
	}
	// 验证当前 display_name 为 V1
	w = doReq(t, r, "GET", "/api/admin/brand", "", sess, csrf)
...
func TestPortalConfig(t *testing.T) {
	...
	r := testRouter(db, t.TempDir())
	sess, csrf := adminSession(t, db)
	doReq(t, r, "PUT", "/api/admin/portal", `{"enabled":true,"welcome":"企业首页","client_download_url":"https://example.com/dl"}`, sess, csrf)
	w := doReq(t, r, "GET", "/api/portal", "", "", "")
