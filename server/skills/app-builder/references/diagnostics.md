...
| `DB_LIMIT` | 是不是「库写满」（507 **只有**这一个含义，外加单行超 8 MiB） | 库满 100 MB 是硬上限（平台不给扩容旋钮）⇒ 清理历史数据或做汇总表。**行数/字节超限不报错**：`db.query` 截断并置 `truncated=true` ⇒ 按它分页（`LIMIT`/`OFFSET`）；语句超时看上一行的 `DB_DENIED` |
