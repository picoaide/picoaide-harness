...
const ROWS = [
  {
    ...
    definition: '{"mcp":[{"serverName":"example-a","transport":"streamable-http","url":"https://mcp-a.example.com/mcp"}]}',
...
describe('Connectors 连接器目录页', () => {
  ...
  it('切换下发开关调用 enabled 端点', async () => {
    ...
    await waitFor(() => {
      expect(mockRequest).toHaveBeenCalledWith('/api/admin/connectors/example-a/enabled', {
        method: 'PUT',
  ...
  it('编辑:预填并保存 PUT', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
    await waitFor(() => {
      expect(mockRequest).toHaveBeenCalledWith('/api/admin/connectors/example-a', {
        method: 'PUT',
  ...
  it('删除:确认后调用 DELETE', async () => {
    ...
    await waitFor(() => {
      expect(mockRequest).toHaveBeenCalledWith('/api/admin/connectors/example-a', { method: 'DELETE' })
    })
  })
})
