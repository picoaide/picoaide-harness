...
- **余额**：`balance_money` 是**存量金额**（元），与月度配额正交。行内「调整余额 / 充值」支持增加 / 扣减 / 设为并写审计；
...
- **部门与角色**：用户归属部门（**支持多部门**，见「部门管理」），部门预算沿所属全部部门+祖先链生效。
...
- **部门预算**（`groups.budget_money`）：**全部**所属部门 + 各自祖先链同时生效（≠ 取最高），`SUM(cost)` 实时统计当月费用（Asia/Shanghai 时区，每月 1 日自动重置）；**任一所属部门预算超限即拦截**（网关 429 `QUOTA_EXCEEDED`，fail-closed：查询失败也拒绝）；部门页进度条实时显示占用%（80% 橙、超限红），继承上级部门预算时显示「继承上级(¥x)」；
