...
产品遵循「技能 + MCP」两种标准形态（2026-08-26 最终架构）：**CLI 厂商能力改由技能商店以 SKILL.md 分发，MCP 能力统一走连接器框架**。早期 CLI 连接器（钉钉/飞书/企业微信/北森等厂商 CLI）已整体移除；当前内置 MCP 连接器为**销售易 NeoCRM** 与一个**远程 MCP 示例**（占位配置，由管理员按实际服务填写），连接器定义可扩展，第三方可注册自己的 MCP def。
