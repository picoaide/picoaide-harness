...
PicoAide Harness 不是「一个有着大量功能的软件」，而是一套**有明确边界的设计哲学**的落地。理解下面的原则，就能预测产品中几乎每一个功能为什么长成现在这个样子。
...
因此 PicoAide Harness 是一个**产品入口和运行时适配层**，不是上游项目的替代品，也不是把上游源码复制一份再长期分叉。
...
- 上游 DeepSeek Harness 以**固定版本原样运行**（当前 pin `dsh-v0.1.6-alpha.2`，真源为仓库根 `upstream.json`），任何产品能力都不修改上游源码；
- 官方生态里的插件可以直接安装使用；
...
- **服务端**：账号体系（local / LDAP / OIDC）、模型网关代理、限流、计量计费与余额闸门、技能与智能体市场、审批、审计——一切「可被滥用」的能力都在服务端；
...
**操作哲学：能收口到服务端的决策，绝不在客户端自证。** 客户端只显示服务端给定的余额与权限结果（如 `429 BALANCE_EXHAUSTED`），不做本地豁免。
...
桌面呈现没有用户可切换的模式。产品固定运行**高级呈现**（advanced）：在不改变上游 Web carrier 的前提下，注入桌面自有的 frame、布局、原生材质与拖动区域；Linux 无平台原生材质（Mica/hidden-inset），使用标准系统窗口边框，但布局与 macOS/Windows 一致。
