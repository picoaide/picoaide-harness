...
> **反过来"服务端升了、客户端没升"是可能的，而它会让应用打不开**：应用（WASM）只在桌面客户端内打开，
> 且**要求客户端与服务端同版本**（浏览器访问链路已删除，旧客户端无法打开应用）。客户端从这台服务器
> 取包，请确认员工接受升级提示、不要长期停留在旧版本。应用本身不需要任何公网入口
> （见[部署总览](/deployment/)）。
...
但**应用（WASM）访问要求客户端升级到位**：停留在旧版本的客户端无法打开应用（见[部署总览](/deployment/)）。
