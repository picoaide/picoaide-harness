---
title: 插件生态
description: DSH 插件生态倡议：开放、可组合、可持续的社区愿景与三条原则；PicoAide Harness 是第一个实践者。
---
...
[DSH Community Fabric](https://github.com/picoaide/picoaide-harness/tree/master/community/fabric) 正在把这份愿景整理成可公开讨论的 Manifest、Capability、Host Descriptor 与事件 Draft。它目前只有文档，不是已经发布的标准或运行时；当前插件仍使用现有 DSH/Cordis 接口。

Fabric 的 capability 首先用于兼容判断、用户确认和审计，不会把同进程 JavaScript 伪装成安全沙箱。只有具备真实隔离证据的 Host 才能声称权限被技术强制执行。

社区市场仍处于文档阶段，尚无可用页面或安装器。目录收录只代表符合目录规则，不等于安全审核或推荐。
