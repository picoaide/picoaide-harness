...
- **Model pricing**: per-model input/output unit price (CNY per M tokens, `input_price_per_1m` / `output_price_per_1m`), plus the **cache-hit input price** (`cache_input_price_per_1m`) and the **off-peak discount rate** (`offpeak_discount`, 0-1) — **models without a price are refused by default** (429 `MODEL_NOT_PRICED`: not forwarded and not charged; for genuinely free/internal models set `gateway.unpriced_model_policy` to `allow`, and only then are they charged as 0); a model with a **zero/very low input price but a normal output price** stays usable (billed on the output side); changing prices/discounts only affects costs incurred afterward (historical costs remain at the pricing recorded at the time);
