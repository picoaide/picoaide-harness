...
export function installPrompts(ctx, config) {
  ...
  //    幂等保护（issue #23）：与 lib/index.js 的 memory:snapshot 同款——宿主
  //    重复装配时 system-prompt global 层同名重复插入抛 "already registered"，
  //    命中则跳过（快照段本就该只有一份），避免整个 apply 失败。
  disposers.push(ctx.effect(() => {
    try {
      ...
      if (error instanceof Error && error.message.includes('already registered')) {
        console.warn('[memory-evolve] prompt:injections 已注册，跳过重复注册（issue #23 幂等保护）')
        return () => {}
      }
      throw error
    }
