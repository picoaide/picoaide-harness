...
渠道客户端（example-a 等白标构建）与官方客户端共用**同一个**数据根：Harness home 是
...
1. **跨租户登录态**：`$DSH_HOME/session.json` 是登录 token，`.credentials.yaml`、
   `settings.yaml`（含服务端地址）、`sessions/` 全在同一目录。装了 example-a 客户端的
   机器上，example-a 客户端会直接恢复官方那台的会话，连上官方服务端。
...
2. **派生唯一入口**：`desktop-home.ts` 的 `channelDshHomeDir(channelId, { homeDir, slug })`。
   取值链：显式值 → slug 小写（`Example-A-Harness` → `.example-a-harness`）→
...
- 真实渠道包（本地 `channels/example-a`、`channels/beta`、`channels/example-brand`）走
...
- **example-a** → `.example-a-harness`（与 slug 派生结果一致，将来即便漏配也不会换目录）；
...
1/3）；用**真实 example-a 渠道包**跑打包门禁 `verify-channel-package`（7/7）与真机探针
—— 数据落到 `~/.example-a-harness` + `~/.config/Example-A Harness`，`example-a-harness://` 的
