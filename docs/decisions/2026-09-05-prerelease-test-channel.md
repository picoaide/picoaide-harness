...
1. 每次测试版 tag 触发三平台构建（mac 为未签名冒烟，约 15～30 分钟），不占公证队列；
   正式版 tag 才触发 mac 签名+公证（约 1～3 小时，含 Apple 公证队列）；
   常见 mac 公证耗时基准：小包 5~20 分钟、Electron 大包 20 分钟~4 小时、异常滞留
   24h+（需联系 Apple Developer Support）；同包高频重复提交会加重 Apple 队列积压，
   重试必须续等同一 submission（状态文件生命周期：公证成功后保留，整条发布链路
   verify 通过后才清除——2026-09-05 rc.3 教训）。
