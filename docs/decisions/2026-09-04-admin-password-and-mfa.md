# 管理员密码修改 + 管理员 MFA（TOTP）

> 状态：已实施（2026-09 全量交付, 见文末实施记录）。分支：master。
> 规划全文(含需求与决策清单)见 docs/planning/2026-09-04-admin-password-and-mfa.md。
...
- **迁移** `0057_admin_mfa.sql`:users 加 password_must_change/password_changed_at/totp_secret/totp_enabled + admin_mfa_challenges 表。
- **服务端**:`POST /api/client/v2/auth/password`(员工自助改密, 全吊销含当前);`POST /api/server/admin/me/password`(改自己密码, 全吊销);`PUT /users/:id` 密码字段置 must_change=1 + 改密时间;`POST /admin/login` 两步改造 + `POST /admin/login/mfa`;`GET/POST /me/mfa`(enable/verify/disable, 主密码+动态码双验);`PUT /users/:id/mfa`(管理员重置他人 MFA, 吊销全部会话);CLI `--reset-mfa <username>` 兜底;BearerAuth/AdminAuth 强制改密守卫 403 `PASSWORD_CHANGE_REQUIRED`(白名单=改密/me/logout);userJSON 补 source/password_changeable/password_must_change/password_changed_at/mfa_enabled。
- **webadmin**:Login.tsx 两步登录状态机;侧边栏「修改密码」「安全设置(MFA 二维码+密钥文本双显)」;用户列表「重置密码」(external 禁用)「重置MFA」按钮 + 「上次改密」列;强制改密拦截(不可关闭)。
- **客户端(enterprise)**:AccountSection 内联改密表单(仅 local 用户, external 显示 IdP 提示);auth-gate 强制改密页 + session 透传 source/must_change_password;locales 中英 key。
- **测试**:Go serverauth 新增 mfa_test.go(改密矩阵/MFA 全流程/挑战守卫/重置/强制改密守卫)+ serverstore DAO + 全量包;webadmin 103 测试(新增 Login 两步流/重置密码/MFA 按钮);enterprise 151 测试(login 解析/changePassword/模板)。make check + yarn check 全绿。
