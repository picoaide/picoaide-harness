...
全仓无任何「保留名 / 锁定 / 仅管理员可发布」机制；员工可以抢占任意未被占用的 App 名（包括 `example-a-*` 这类官方命名）。
...
- **支持预锁定**：可创建一个没有任何 release 的 App 占名（如提前锁定 `example-a-*` 系列官方技能名），防止员工抢占。
...
> 说明：本轮不引入前缀通配（如 `example-a-*` 批量保留）。预锁定已能覆盖「保护官方命名」的诉求，通配规则会让「谁能发」的判定变得难以预测，列为未来可选项。
