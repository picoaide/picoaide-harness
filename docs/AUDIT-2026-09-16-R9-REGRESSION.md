...
| R8 | 1 个对抗复核代理（收尾确认轮） | **CONVERGED**：四处修复正确/完整/零放宽/零误伤；三条不变量复现（base 非空→null=0、强档 603.5 万组逐条一致、与 R7 版"只收紧"）；无 P2+ |
...
- 继续加固的边际收益很小，而**回归风险已被实测兑现**（R7 的 P2 正是 R6 那一轮加固的
  副作用）；
- 因此判据冻结。**冻结范围 = "裸主机猜测判据"**（`looksLikeHostText` 守的那条路径）：
  显式 http(s) URL（`httpOriginOf`）与 `credentialSites` 声明路径自 base 起逐字未变，
  它们仍接受 `x.localhost`/`localhost.com`/`a%2eb.com` 这类拼写 —— 记录级抽检 7,435 个
  "点分 localhost 绑定"全部来自这两条路径，判据闭合成立（R8 的范围口径）。
...
| 残余 | 形态 | 影响 | 为什么不再加固 |
| --- | --- | --- | --- |
| 未枚举的通用占位词 | `mycompany.com`/`acme.com`/`site.com` 等 45 个**真实可解析**的占位域名写在地址键里 | 会压过同记录的真站点 URL；拒绝文案会把该主机当导航指令（模型照做后闸门会放行，凭据是该运维自己的） | R6 起行为未变、非本轮引入；否表有界而该类无界（R4-3 先例记 P3） |
| 低置信 origin 的**文案**仍像指令 | `tools.ts:105-115` 的 "navigate the tab to … first" 对"猜出来的" origin 与"显式声明的"不作区分 | 模型可能被引导到占位主机 | 需要给解析器加"来源"维度（plumbing 改动），列为后续建议，**尚未实施**（本节早期措辞曾误称已实施，R8-2 更正） |
| 显式 URL / `credentialSites` 路径 | `x.localhost`、`a%2eb.com` 等作为**显式 URL** 出现时照旧放行 | 与 base 逐字一致 | 越出"回归修复"范围；改动会改写既有绑定 |
