...
> ⚠️ **部分作废（2026-09-19）**：本文的发现与处置是审计当时的结论；其中以 `anonlimit`（匿名限流）
> 与 `aichat`（服务端 AI）为对象的条目，因这两个包随「客户端专属」改造整包删除而**不再适用**
> （总纲 §8.4 / §21.3），已在原处就地标注；其余条目仍为现役结论。
...
| ~~P2-4~~ | ~~匿名限流拒绝数、aichat 吊销失败数、库句柄数等水位零出口~~ **部分已废弃（2026-09-19）**：其中 `anon_limit` 与 `aichat` 两项的**对象已随 W4 整包删除**（总纲 §8.4 / §21.3）⇒ 这两项水位不再存在；**库句柄数等其余水位项仍成立** | 修（并入 P1-9 的诊断面；`anon_limit`/`aichat` 两项随之作废，应用 AI 改看应用维度用量面板，总纲 §21.4） |
