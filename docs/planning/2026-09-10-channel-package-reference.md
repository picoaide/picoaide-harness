...
**还必须画出方块之外的 mark**（2026-09-16 新增，同样是打包期硬约束）：macOS 的
`tray-iconTemplate*.png` 是**模板图**（AppKit 只拿 alpha 通道当遮罩），所以派生时会
**整块删掉那个 `<rect>` 底板**、只留 mark 并规整成黑色 —— 否则菜单栏里就是一个实心
方块（暗色菜单栏上是白方块），花括号/mark 永远显示不出来。渠道 logo 若只有一块纯色
方块、方块之外什么都没有，模板图会全透明（= 菜单栏里什么都没有），构建直接 fail-loud
并点名 mark。Windows/Linux 位图不受影响，仍是"方块 + mark"的完整品牌图形。
