...
> ⚠️ **回滚镜像会连带回滚"客户端可升级到哪个版本"**（2026-09-12 补）：
> `releases/<v>/` 里**含客户端安装包**，`latest.json` 与 `GET /api/client/v2/updates/manifest`
> 由当前运行镜像内嵌的清单决定。把服务端镜像回滚到 v_old 之后：
> ① 客户端看到的"最新可用版本"随之倒退；② 客户端的更新检查只认**严格大于**当前版本
> （`update-checker.ts`），所以已经在 v_new 的客户端不会降级，只会显示"已是最新" —— 
> **它们会长期停在比服务端清单更新的版本上，而没有任何提示**（没有 `min_client_version`
> 这类强制升级闸门）。
> 因此：回滚前先确认"已升级的客户端版本与服务端 API 仍然兼容"；若服务端回滚跨越了
> 客户端契约变更（例如 0061/0062 余额字段、DSH 会话格式世代），必须同时通知客户
> **不要降级客户端**，并按 `docs/deploy/AI-DEPLOY.md` §7 的口径处理。
...
### 10.2 桶是**公开读**的：渠道 id 就是公开路径前缀

`release.picoaide.com` 是 R2 自定义域，对象**无需任何凭证即可读取**（2026-09-10
实测：匿名 `curl .../beta/latest.json` → 200）。因此：

- **渠道 id = 可被枚举/猜测的公网路径前缀**。官方/beta 无所谓；**品牌渠道
  必须用不可猜的 id**（例如带随机后缀），否则"知道客户名 → 拿到客户镜像"——
  镜像内含该客户渠道的品牌素材、文案与客户端安装包。
- 品牌渠道的**客户端安装包**已不再经公开 GitHub artifact（见 §10.1），但品牌
  **镜像**仍落在 `<channel-id>/` 下：这是当前的分发形态，安全边界落在"渠道 id
  不可猜"上。若将来要求更强隔离，改私有桶 + 预签名 URL（`aws s3 presign`，
  有效期可设 7 天）或 Cloudflare Access。
- 渠道 id 出现在这些公开处：R2 路径、镜像内 `/opt/picoaide/CHANNEL` 与
  `channel.json`、`GET /api/client/v2/channel`（客户端对账需要）、门户页脚、
  被掩码后的 Actions 日志（`::add-mask::` 覆盖）。**不要把客户真名当渠道 id**。
...
**实测数据**（测试环境 198.51.100.10，目标包 616,752,285 B）：
