...
- 方法：设计文档对照（`docs/planning/2026-09-07-ai-browser-redesign.md`）→ 全量源码走读 → 单测补丁实锤（`vitest`，158 存量 + 5 项探针全绿）→ 真机行为验证（https://harness-legacy.example.com/ user001，CDP + 真实合成截图 `import -window root` + 真实鼠标点击 `xdotool`）
...
## 真机验证记录（2026-09-07，harness-legacy.example.com / user001）
...
- 真机 E2E（harness-legacy.example.com / user001，重建 dist/linux-unpacked + `temp/browser-v4.2-e2e.mjs`）：**14/14 通过**，含真实合成截图断言——标签打开后胶囊仍可见（v42-01）、活动面板浮层真实可见（v42-03）、接管态（v42-04）、菜单/查看器（v42-05/06）——CDP 盲区已纳入验证。
