...
> **2026-09-20 订正（随包资源不再落盘）**：本节的"发布期把自定义段抽到
> `<data_root>/apps/<app_id>/assets/<release_id>/`、运行期从该目录读"这套模型**已作废**。
> 现在随包资源在运行期从制品字节解析后**常驻内存直出**，宿主盘上不再有版本资源目录；
> 权威口径见 `docs/decisions/2026-09-20-wasm-assets-in-memory.md`。本节其余部分
> （请求管线、客户端形态、准入与安全模型）仍然有效。
...
语义（**冻结，逐条要有判据**）：
- **读源（2026-09-20 第三轮审计后定稿为"同一天同源"）**：`apps[]` / `today` / `totals` 读**明细表** `wasm_app_opens`（与 §5.1b 的 `opens.today` 同源，保证"本次调用计数在内"）。`trend[]` **按天判**：**明细覆盖得到的天，PV 与 UV 都从明细取**（一次聚合同时得到 `count(*)` 与 `count(DISTINCT user_id)`）；**只有明细已不在的天**（早于 90 天保留期）才把 **PV** 回落日汇总（UV 如实 0）。日期集合 = **明细天 ∪ 日汇总天**（否则首个 5 min tick 之前曲线会漏掉"今天"，而 `today` 已经有数）。**两个读源并存是有意的，不是 bug**；但**同一天的两个数不得来自不同读源** —— 那会让同一响应自相矛盾。
- **UV 一律真实去重**：`uv` = `count(DISTINCT user_id)`。**禁止**把日汇总的逐日 `uv` 相加，**禁止**把各应用的 `uv` 相加（同一个人开两个应用会被重复计）⇒ `totals.uv` / `today.uv` 必须是**不带 `GROUP BY app_id` 的一次聚合**。这是 W5 C2 既有禁令的延伸。
  - **本条同样约束 `trend[]` 的每一个点**（2026-09-20 主控裁定，修 P1-1）：日汇总表主键是 `(app_id, day, dept_id)` —— 它**结构上给不出**跨应用/跨部门的当日去重人数，从它取 UV 只有 `SUM(uv)` 一条路，而那正是本条禁止的做法。因此 `trend[].uv` **改读明细**（`count(DISTINCT user_id)` 按当地日分组）；实测同一份数据下旧口径 `SUM(uv)=6`、新口径 `=2`，与同页 `today.uv` / `totals.uv` 一致。**不许"为了曲线不断档"就在 UV 上退化成相加**。
  - **不变量：`uv <= pv` 对每个 day 点恒成立**（2026-09-20 第三轮审计发现并修）：第一版把"PV 读日汇总、UV 读明细"混用，于是 5 min tick 落后时同一响应出现 `trend={pv:26,uv:27}`（去重人数大于打开次数，语义上不可能）——活体复现过。**修法即上一条的"同一天同源"**，并把不变量写成判据（另加"`today` 有数 ⇒ 曲线必有今天且逐值一致"）。
  - **日边界必须单源**：逐日边界由 Go 侧按部署时区（`LocalDay`）算好后**一次性**传给 SQL（`width_bucket(floor(extract(epoch))::bigint, ?::bigint[])`），**不得**在 SQL 里写 `opened_at::date` 或 `AT TIME ZONE` —— 后者会让"日"的定义出现第二份实现，且在 DST 的 23/25 小时日上错位（已有 `TZ=America/New_York` 的 DST 判据）。
  - **保留期边界（认账）**：明细只保留 90 天，故窗口内**早于保留期**的日期 UV 如实给 **0**（PV 仍真实，来自日汇总），且请求窗口长于保留期时按既有 `capped=true` 如实回报；**不得**用 `SUM(uv)` 顶替。长期逐日去重 UV（日汇总加一条 `(day)` 维度的全局去重行 + 写路径变更）**需要迁移**，未在本版实施，已登记为候选（§9）。
...
**冻结/下架/删除的三档呈现（冻结，产品第四轮 Q2/Q3）**：①**目录**：**冻结不列**（冻结不进目录，§19 Q3）；**已下架仍列**并带「已下架」标记 —— 与 F1 的"上下架状态"列、§19 Q2② 的"全部下架"提示三者一致；**删除**的既不在目录也无法打开（§19 Q3/§7.7②）。⚠️ **主控 2026-09-20 订正**：本行原写"冻结与已下架**都不列**"，与 F1/§19 Q2 自相矛盾（那是我上一轮把冻结规则错误地扩到了下架），**以本行为准**；②**直接打开**（深链/历史）：冻结 ⇒ 「已被管理员停用」+ 联系负责人；下架 ⇒ 「已下架」；删除 ⇒ 「应用不存在」；③**内层 404 必须带 `reason`**，且取值是**封闭两档**（2026-09-20 主控收紧，与 L1 实施逐字一致）：`app_frozen`（只读快照，数据保留）与 `app_not_found`（**软删与"从未登记"同档同 reason**）；**下架由 HTTP 410 承载**，不占 404 的 reason。客户端据此选文案 —— 三档**不得**塌缩成同一句话；宿主把平台拒绝封装回页面时**必须同时透出 `platform_code` 与原 `platform_reason`**，客户端**两处都要读**（只读 `platform_code` 会让冻结塌回"应用不存在"，2026-09-20 实测缺陷）。
