...
PicoAide Harness 是这套方式的第一个实践者：桌面壳本身就是一个普通 DSH 插件，与官方、第三方插件走同一条组合路径，没有任何特权。我们不是魔改上游源码做一个固定外壳，而是让"桌面"也成为插件生态里平等的一员。
...
[DSH Community Fabric](../community/fabric/README.zh.md) 正在把这份愿景整理成可公开讨论的 Manifest、Capability、Host Descriptor 与事件 Draft。它目前只有文档，不是已经发布的标准或运行时；当前插件仍使用现有 DSH/Cordis 接口。

Fabric 的 capability 首先用于兼容判断、用户确认和审计，不会把同进程 JavaScript 伪装成安全沙箱。只有具备真实隔离证据的 Host 才能声称权限被技术强制执行。

社区市场仍处于文档阶段，尚无可用页面或安装器；相关讨论见 [DSH Community Fabric Draft](../community/fabric/README.zh.md)。目录收录只代表符合目录规则，不等于安全审核或推荐。

## 如何参与

- 在[插件开发](plugin-development.md)中了解插件如何编写。
- 阅读并评论 [Community Fabric RFC 0001](../community/fabric/docs/rfcs/0001-plugin-manifest-capabilities-events.zh.md)。
