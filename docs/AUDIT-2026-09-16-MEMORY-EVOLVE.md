...
| 变化 | 谁受影响 | 说明 |
|---|---|---|
| `memory action=list target=key` 缺省按**当前分支**过滤 | 用 git 分支记忆作用域的用户 | 此前能列出仅限其它分支的条目（那些条目既不会注入、也 `expand` 不出来）；现在与注入/expand 一致。要跨分支查看就显式传 `branch`；面板逃生口＝「Memory Evolve 设置 → key 轨分支过滤」开关（2026-09-16 对抗复核后已把它加进运行时键，此前只有 cordis 行 config 能改、桌面分发里够不到） |
...
D 批提交后由独立审查者以"找真 bug"角度复核（含用 esbuild 重跑重建、验证产物逐字节相同），
结论**未发现真回归**，但点出 6 条真实缺陷，**全部已修并补回归**：A1 留档只在 console.warn、
用户看不出设置被重置（→ 标记文件 + 快照一次性告知 + 宿主日志 + 留档上限 3 份）；
A2 `keyBranchFilter` 逃生口在桌面端不可达（→ 进 RUNTIME_KEYS + 面板开关）；
A3 `clientLang()` 严格匹配与 D10 口径不一致（→ 已论证等价，保留并加注释）；
A4 一条回归在非 git 目录上假绿（→ 改真 git 反例）；A5 D5 的承重接线零覆盖（→ 补断言）；
A6 日志第二参打成 `[object Object]`（→ 内联）。明细见 `VENDORED.md` §E。
