...
- On the enterprise feature branch, feature work must only edit `plugins/dsh-enterprise/` and `server/`. No changes to other service packages or to `deepseek-harness/`; test adaptations in desktop-owned scripts are allowed.
