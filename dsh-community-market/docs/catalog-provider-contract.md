...
- source selection, ordering, aggregation, provenance, and failure behavior;
...
| Source record ID | An opaque UUID generated by the Host for one local source registration. Cache, cursors, aggregation, and item identity use this ID. |
...
3. enabling and disabling each source independently;
4. reordering enabled sources;
...
No manifest may declare itself enabled, first, trusted, official, recommended, or a fallback. The source schema intentionally rejects `enabled`, `order`, `priority`, authentication material, custom headers, scripts, and install commands. A built-in source and a user-added source may claim the same `providerId` without sharing identity, cache, cursor, trust, or presentation. The local adapter registration—not that claim—controls whether a reviewed partner badge is shown.

On first run, the market may present available source choices, including cooperating providers, but it must not preselect them. With zero enabled sources, the UI shows an explicit “choose or add a source” state, sends no catalog requests, and never silently switches to DSH 1024Store or any other provider.

Changing source order affects presentation only. It does not change trust, validation, installation permissions, or conflict resolution.

The first implementation can persist this private shape (it is not accepted from the network):

```ts
interface LocalSourceRecord {
  sourceRecordId: string // Host-generated UUID
...
```mermaid
flowchart LR
    Settings["Local source settings<br/>enabled and ordered by the user"] --> Registry["Source registry"]
    ...
    Builtin --> Validate
    Validate --> Aggregate["Multi-source aggregator<br/>provenance preserved"]
    Aggregate --> UI["Market UI and confirmed install boundary"]
```
...
Every successful result is validated against [`catalog-snapshot.schema.json`](schemas/catalog-snapshot.schema.json) before it can be cached, aggregated, displayed, or considered for installation.
...
| `limit` | zero or one | Integer from 1 through 100. The normalized Host query defaults to 20; the effective requested value cannot exceed the manifest's `maxLimit`. |
...
The aggregator works on independent source results, not on one assumed global catalog:

- Enabled sources are requested concurrently with a bounded concurrency limit.
- Each source has its own timeout, cancellation, cache entry, cursor, loading state, and error state.
- One source failing does not discard valid results from another source. The UI shows available items and a compact error/retry state for failed sources.
- Disabling or removing a source cancels its in-flight work and removes its results without restarting DSH.
- User-selected source order determines source sections and deterministic tie-breaking only.
- Every card, detail view, search result, and install confirmation keeps visible source attribution.

The canonical item identity is the pair `{ sourceRecordId, itemId }`. Two registrations—even for the same provider—remain distinct and may legitimately describe the same plugin differently. The Host may create a presentation group only when items share either:

1. the same canonical npm package identity; or
2. the same canonical repository URL and subdirectory.

Grouping is not merging. Every variant, provenance record, compatibility claim, version, description, and source attribution remains available. No source silently overwrites another source's claim. A name, repository name, or similar description alone is never enough to deduplicate items.

If grouped variants disagree, the UI reports the disagreement and asks the user which source record to use. Source order must not turn a disagreement into an implicit security or installation decision.
...
This relationship makes 1024Store a supported source choice. It does **not** make it the default, preferred, official, recommended, audited, or fallback source. The adapter does not auto-enable it, and an empty or failed source list never triggers a hidden request to it. Its catalog remains an independent project, and catalog inclusion is not a security review of any listed plugin.
...
- Adding, enabling, disabling, reordering, or removing a source requires an explicit user action.
- Remote manifests cannot modify local source settings or add another source.
- Persist source settings separately from cached remote data. A cache refresh cannot change enabled state or order.
...
The implementation team can start contract tests with the matching [source manifest](examples/catalog-source.example.json), [query](examples/catalog-query.example.json), [provider page](examples/catalog-provider-page.example.json), and [normalized snapshot](examples/catalog-snapshot.example.json) fixtures. They are examples only: the source fixture is not a bundled or enabled provider, and none of these files is runtime configuration.
...
4. Build one validated query, derive each source's supported query, and schedule enabled sources with bounded concurrency.
5. Fetch, validate, normalize, and cache each source independently.
6. Aggregate successful snapshots while preserving partial errors and provenance.
7. Cancel owned requests when the query changes, a source is disabled, the plugin generation is disposed, or DSH shuts down.

The first implementation may choose cache duration and concurrency values, but it must keep the independence, cancellation, no-default, and no-fallback behavior defined here.
...
- [ ] Persist user-owned source records with a Host-generated UUID, adapter identity, manifest URL or built-in provider ID, registration kind, enabled state, and order.
- [ ] Implement add, inspect, enable, disable, reorder, retry, and remove actions.
- [ ] Ship with no source selected and an explicit zero-source state.
- [ ] Show attribution and endpoint host before enabling a source and on every result/detail/install surface.
- [ ] Make errors source-specific; do not replace a failed source or reorder sources automatically.
...
- [ ] Add bounded concurrency, per-source abort/timeout/cache/pagination, and partial-failure aggregation.
...
- [ ] Require explicit source-variant selection when grouped records disagree.
...
| --- | --- | --- |
| No default | Fresh profile has zero enabled sources | Explicit source-choice empty state; no network request and no fallback |
| Selection | User enables and reorders two sources | Order persists locally and affects presentation only |
| Selection | Remote manifest contains `enabled`, `priority`, auth, header, script, or install fields | Manifest rejected by the strict schema |
| Selection | DSH 1024Store adapter is available on first run | It is visible as a choice but remains disabled until selected |
...
| Aggregation | User changes source order during a request | Obsolete aggregate work is cancelled; new order does not alter trust |
...
| Lifecycle | Source is disabled or Host disposed during fetch | Fetch aborts, resources release, and no late result mutates state |
...
| Install | User chooses one of conflicting source variants | Confirmation shows exact source, identity, and active profile before execution |

## Open implementation details

The implementation team may propose concrete cache TTLs, concurrency counts, byte/item budgets, locale fallback behavior, and UI layout during review. Those choices must be documented and tested. They may not weaken the explicit-selection, no-default, no-fallback, strict-validation, provenance, partial-failure, or non-executable-data rules without revising this contract.
