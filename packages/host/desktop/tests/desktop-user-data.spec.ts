...
describe('desktopUserDataDirectoryName (随渠道的第二份数据根)', () => {
  ...
  it('keeps the brand channel product name as-is', () => {
    ...
    expect(desktopUserDataDirectoryName('Example-A Harness', 'example-a')).toBe('Example-A Harness')
  })
  ...
  it('never returns an empty or path-like name for a channel', () => {
    for (const id of ['example-a', 'beta', 'acme-2']) {
...
describe('defaultDesktopUserDataDirectory (无 Electron 的等价实现)', () => {
  ...
  it('uses the channel-specific name when one is given', () => {
    ...
    expect(defaultDesktopUserDataDirectory('linux', { XDG_CONFIG_HOME: '/home/example/.config' }, '/home/example', 'Example-A Harness'))
      .toBe('/home/example/.config/Example-A Harness')
  })
