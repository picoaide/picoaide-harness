...
`scripts/verify-packaged-runtime.ts` runs as Electron Builder's `afterPack` hook on all three platforms, so a package that cannot run its own runtime fails to build instead of shipping. Next to the static entry lists it spawns the packaged launcher in Node mode (`ELECTRON_RUN_AS_NODE=1`) for four behavioural smokes: the diagnostics worker's crash dump, the POSIX session `flock` binding, the enterprise error-reporting module, and — since 2026-09-23 — the **`app.asar` `{ bigint: true }` semantics and the filesystem skill listing they gate** (issue #130).
...
**Upgrading or downgrading Electron requires re-running this gate** — `node scripts/package-dir.mjs` (or `yarn dist:linux`) runs `afterPack` for real. `tests/package.spec.ts` additionally pins the `electron` dev/peer pair to an exact version with major ≥ 44; that floor now carries the ASAR contract above as well as the Chromium security backports, so do not relax it without re-reading this section. To reproduce the failure mode against an arbitrary engine without touching the gate:
