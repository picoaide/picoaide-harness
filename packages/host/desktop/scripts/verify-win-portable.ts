...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `PicoAide-Harness-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'PicoAide Harness.exe')
  ...
  assertPortableExecutableBuffer(
    ...
    `${portablePath}:PicoAide Harness.exe`,
  )
  return portablePath
}
