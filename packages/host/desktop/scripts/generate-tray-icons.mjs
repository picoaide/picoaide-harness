...
if (!source.includes(`fill="${BRAND_COLOR}"`) || /<style\b/iu.test(source)) {
  throw new Error(`generate-tray-icons: tray-icon.svg must use the fixed brand color ${BRAND_COLOR}`)
}

const variants = [
  ['tray-iconTemplate.png', '#000000', 16],
  ['tray-iconTemplate@2x.png', '#000000', 32],
  ['tray-icon-blue.png', BRAND_COLOR, 16],
  ['tray-icon-blue@1.25x.png', BRAND_COLOR, 20],
  ['tray-icon-blue@1.5x.png', BRAND_COLOR, 24],
  ['tray-icon-blue@2x.png', BRAND_COLOR, 32],
]

await Promise.all(variants.map(async ([filename, color, size]) => {
  const rendered = source.replaceAll(BRAND_COLOR, color)
