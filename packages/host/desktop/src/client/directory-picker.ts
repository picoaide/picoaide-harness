...
export async function requestDesktopDirectory(
  ...
  if (!response.ok) throw new Error('Could not open the system folder picker')
  const value: unknown = await response.json()
  if (!isResponse(value)) throw new Error('The system folder picker returned an invalid response')
  return value.path
}
