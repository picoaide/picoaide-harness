/**
 * R4-RV3a（第四轮收尾）：desktop 写面的 BrowserAuth 持有性证明。
 *
 * `desktop-update-route.ts` 的 `acceptRendererPost` 只在
 * `origin !== undefined && origin !== expectedOrigin` 时拒绝 —— **不带 Origin
 * 直接过**。本机任意进程伪造 `Host`/`Origin`/`Sec-Fetch-Site` 即可
 * `POST /api/pico/desktop/update/install` 拉起已下载的安装包、`/update/check`
 * 触发一次联网检查；`/_dsh/desktop/renderer-boot` 与 `pick-directory` 同族。
 *
 ...
 * @module dsh-plugin-desktop/write-proof
 */
 ...
 *
 * 每个写路由自己调用本函数（而不是由注册方统一包装），因此任何调用者——包括
 * 直接调用 handler 的代码——都无法绕过证明；`proof` 未接线（undefined）按证明
 * 机制缺席 fail-closed，不退回各路由自己的 Origin 检查。
 * @param req - 进入证明闸的请求。
 * @param res - 拒绝时写出的响应。
 * @param proof - 证明依赖；缺省/未接线 ⇒ 503。
 * @returns 请求持有证明、可以继续时为 true。
 */
export function acceptWriteProof(
  req: IncomingMessage,
  res: ServerResponse,
  proof: WriteProofDeps | undefined,
): boolean {
  const outcome = requireWriteProof(req, proof ?? { fence: (): undefined => undefined, label: 'dsh-plugin-desktop' })
  if (outcome.ok) return true
  res.statusCode = outcome.status
  res.setHeader('content-type', 'application/json; charset=utf-8')
  res.end(JSON.stringify({ error: outcome.error, hint: WRITE_PROOF_HINT }))
  return false
}
