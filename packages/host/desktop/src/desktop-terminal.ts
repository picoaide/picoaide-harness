...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`PicoAide Harness ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart PicoAide Harness after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("PicoAide Harness {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart PicoAide Harness after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(PicoAide Harness !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart PicoAide Harness after plugin changes.')}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "PicoAide Harness" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
