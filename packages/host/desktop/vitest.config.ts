 ...
 * **这个代价是认账的**（第十轮复审 N10 实测：副本内注入一条永不结算的用例
 * `Test timed out in 30000ms`、`real 0m31.123s`）。反方向的代价更大：等待预算不可达
 * 会让门禁随机假红，团队被训练成"rerun 一下就好"（第九轮的 `--body fileb://` 就是
 * 这样活到 tag 的）。
 *
 * **口径唯一**：本文件是"包级缺省 = 30s"的唯一定义点，`tests/wait-budgets.ts` 只登记
 * 另外显式声明的预算并读这里取值（第十轮修复第一版两处文字互相矛盾，复审 N9 已收口）。
 *
 * ### 三条规矩
 *
