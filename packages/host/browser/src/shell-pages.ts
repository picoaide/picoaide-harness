...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .tab .favicon { width: 16px; height: 16px; border-radius: 3px; background: var(--surface-hover); flex: none; }
  ...
  /* ---------- AI indicator ---------- */
  #ai-indicator { position: fixed; right: 16px; bottom: 16px; display: flex; align-items: center; gap: 6px; height: 30px; padding: 0 12px; border-radius: 999px; border: 1px solid var(--border); background: var(--surface-raised); box-shadow: 0 4px 16px rgba(0,0,0,.12); cursor: pointer; z-index: 40; }
  #ai-indicator .dot { width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); flex: none; }
  #ai-indicator .dot.busy { background: var(--accent); animation: breathe 2.4s ease-in-out infinite; }
  #ai-indicator .dot.paused { background: var(--warning); }
  #ai-indicator .label { max-width: 220px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  #ai-indicator .take { display: none; margin-left: 2px; height: 24px; padding: 0 10px; font-size: 12px; background: var(--warning); border-color: var(--warning); color: #fff; }
  #ai-indicator:hover .take { display: inline-flex; align-items: center; }

  /* ---------- Activity panel ---------- */
  #panel { position: fixed; top: 0; right: 0; bottom: 0; width: 340px; z-index: 50; background: var(--surface-raised); border-left: 1px solid var(--border); box-shadow: -8px 0 28px rgba(0,0,0,.10); transform: translateX(105%); transition: transform 200ms ease-out; display: flex; flex-direction: column; }
...
<script>
  ...
  const state = { tabs: [], controlled: false, busy: false, busyTool: '', ops: [] }
  let sseOk = false
  ...
  function render() {
    ...
    for (const tab of state.tabs) {
      ...
      el.addEventListener('click', (e) => {
        ...
        if (!tab.visible) post('switch-tab', { tab: tab.id })
      })
  ...
  function renderStream() {
    ...
    const take = $('take-btn')
    if (state.controlled) {
      take.textContent = '交给 AI'; take.className = ''; take.style.background = 'var(--accent)'; take.style.borderColor = 'var(--accent)'; take.style.color = '#fff'
  ...
  $('go-back').addEventListener('click', () => post('back').then(refresh))
  $('go-forward').addEventListener('click', () => post('forward').then(refresh))
  $('reload').addEventListener('click', () => post('reload').then(refresh))
  ...
  const go = () => {
    ...
    window.__lastNav = normalized
    post('navigate', { url: normalized }).then(refresh)
  }
  ...
  $('menu-btn').addEventListener('click', (e) => {
    ...
    const items = [
      ...
      { sep: true },
      { label: '清除数据…', danger: true, action: () => { if (confirm('清除全部浏览数据（含登录状态）？')) post('clear-data').then(refresh) } },
  ...
  document.addEventListener('click', (e) => { if (!e.target.closest('#menu') && !e.target.closest('#menu-btn')) closeMenu() })
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.ctrlKey && e.key.toLowerCase() === 'w') { e.preventDefault(); const t = state.tabs.find((x) => x.visible); if (t) post('close-tab', { tab: t.id }) }
...
<style>
  ...
  @media (prefers-color-scheme: dark) {
    body { background: rgba(0, 0, 0, 0.14); }
  }
  #pill {
    display: flex; align-items: center; gap: 8px;
    padding: 7px 14px; border-radius: 999px;
    background: rgba(30, 34, 42, 0.86); color: #fff;
    box-shadow: 0 4px 16px rgba(0,0,0,.22);
    max-width: 80%; overflow: hidden; white-space: nowrap;
  }
  #dot { width: 8px; height: 8px; border-radius: 50%; background: #4f83ff; animation: breathe 1.6s ease-in-out infinite; }
  #txt { overflow: hidden; text-overflow: ellipsis; }
  #hint { color: rgba(255,255,255,.75); font-weight: 500; }
...
<script>
  var state = { controlled: false }
  var TITLE = ''
  function labelOf(tool) {
    var map = { browser_open: '打开标签页', browser_navigate: '打开网页', browser_click: '点击', browser_type: '输入', browser_press: '按键', browser_select: '选择', browser_scroll: '滚动', browser_screenshot: '截图', browser_get_snapshot: '读取页面', browser_get_text: '读取文字', browser_eval: '读取页面数据', browser_wait_for: '等待页面', browser_fill_form: '填写表单', browser_fill_credentials: '填写登录', browser_upload_file: '上传文件', browser_download: '下载', browser_bookmarks_add: '收藏' }
    return map[tool] || tool || '操作'
  }
  function render() {
    var txt = document.getElementById('txt')
    if (state.controlled) { txt.textContent = '你正在操作'; return }
    var label = TITLE || 'AI 正在操作'
    if (state.busyTool) label += ' · ' + labelOf(state.busyTool)
    txt.textContent = label
  }
  function poll() {
    fetch('/api/pico/browser/state').then(function (r) { return r.json() }).then(function (d) {
      state.controlled = d.controlled === true
      state.busyTool = d.busyTool || ''
      TITLE = d.busy === true
        ? 'AI 正在操作' + (labelOf(state.busyTool) !== state.busyTool ? ' · ' + labelOf(state.busyTool) : '')
        : ''
      render()
    }).catch(function () {})
  }
  document.body.addEventListener('click', function () {
    fetch('/api/pico/browser/takeover', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ active: true }) }).then(function () { poll() }).catch(function () {})
  })
  poll()
