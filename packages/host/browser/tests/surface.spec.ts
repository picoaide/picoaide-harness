...
describe('app scheme lookup by URL (浏览器面拒绝时的定位，§22.2 R4)', () => {
  it('finds the surface whose channel-injected scheme the URL uses', () => {
    const { registry } = harness()
    const surface = registry.registerApp({ id: 10_001, appId: 'my-notes', appScheme: 'gentech-harness-app' })
    expect(appSurfaceForUrl('gentech-harness-app://my-notes/', registry)).toEqual(surface)
