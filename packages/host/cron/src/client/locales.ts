...
export const zh = {
  ...
  'settings.timeZoneDrift': '⚠️ 系统时区已变更：宿主记录的时区是 {host}，本机现在是 {local}。下面的「下次运行」已按本机时区重算；宿主会在下一次 tick（约 30 秒内）按新时区重排，重排后触发点与表达式一致。',
...
export const en: Record<CronKey, string> = {
  ...
  'settings.timeZoneDrift': '⚠️ The system timezone changed: the Host recorded {host}, this machine is now {local}. The next-run times below are recomputed in the local timezone; the Host re-anchors them within one tick (~30s), after which the trigger matches the expression again.',
