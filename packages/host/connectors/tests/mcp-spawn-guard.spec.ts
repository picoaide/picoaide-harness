...
describe('FIX-02 parseServerConnectors: catalog entries are validated before use', () => {
  ...
  it('keeps the shipped seed definitions (https MCP + npx stdio)', () => {
    const defs = parseServerConnectors([
      row('example-a', {
        auth: { discoveryUrl: 'https://mcp-a.example.com/mcp', clientId: '', authorizeUrl: '', tokenUrl: '', redirectUri: 'http://127.0.0.1/callback', pkce: true, publicClient: true, scopes: 'offline_access' },
