...
const EXAMPLE-A_DEF = JSON.stringify({
  auth: {
    discoveryUrl: 'https://mcp-a.example.com/mcp',
    clientId: '',
    authorizeUrl: '',
    tokenUrl: '',
  ...
  mcp: [{ serverName: 'example-a', transport: 'streamable-http', url: 'https://mcp-a.example.com/mcp' }],
})
...
describe('parseServerConnectors', () => {
  it('解析 example-a: 目录行字段覆盖 + auth/mcp 完整保留', () => {
    const defs = parseServerConnectors([
      { id: 'example-a', name: '示例 MCP 智能体', description: '招聘与人事', auth_mode: 'oauth', definition: EXAMPLE-A_DEF },
    ])
    expect(defs).toHaveLength(1)
    const m = defs[0]!
    expect(m.id).toBe('example-a')
    expect(m.name).toBe('示例 MCP 智能体')
    expect(m.authMode).toBe('oauth')
    expect(m.auth?.discoveryUrl).toBe('https://mcp-a.example.com/mcp')
    expect(m.auth?.pkce).toBe(true)
    expect(m.auth?.publicClient).toBe(true)
    expect(m.mcp).toEqual([{ serverName: 'example-a', transport: 'streamable-http', url: 'https://mcp-a.example.com/mcp' }])
  })
  ...
  it('目录行缺 auth_mode 时回退定义 JSON 的 authMode', () => {
    const defs = parseServerConnectors([
      { id: 'example-a', name: '', description: '', auth_mode: '', definition: EXAMPLE-A_DEF },
    ])
