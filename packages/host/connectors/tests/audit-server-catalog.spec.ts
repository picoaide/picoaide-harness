 ...
 */
const REAL_CATALOG = [
    {"id": "example-a", "name": "示例 MCP 智能体", "description": "远程 MCP 连接器示例:OAuth 2.1 + PKCE + 授权服务器元数据发现;名称、描述与端点由管理员按实际服务填写。", "auth_mode": "oauth", "definition": "{\"auth\":{\"discoveryUrl\":\"https://mcp.example.com/mcp\",\"clientId\":\"\",\"authorizeUrl\":\"\",\"tokenUrl\":\"\",\"redirectUri\":\"http://127.0.0.1/callback\",\"pkce\":true,\"publicClient\":true,\"scopes\":\"offline_access\"},\"mcp\":[{\"serverName\":\"example-a\",\"transport\":\"streamable-http\",\"url\":\"https://mcp.example.com/mcp\"}]}"},
...
describe('server catalog contract (real definitions)', () => {
  it('parses every enabled definition the server actually ships', () => {
    ...
    expect(example-a?.mcp[0]?.url).toBe('https://mcp.example.com/mcp')
    expect((example-a?.auth as { discoveryUrl?: string })?.discoveryUrl).toBe('https://mcp.example.com/mcp')
