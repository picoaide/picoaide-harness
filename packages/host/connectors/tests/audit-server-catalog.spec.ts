 ...
 * environment's `connectors` table, enabled rows only). 客户自有 MCP 端点已按仓库规则
 * 换成占位符主机 `mcp.example.com` —— 真实端点属私有仓 `picoaide/channels` 与部署侧
 * 配置，**不得**写回本文件（`node scripts/check-no-real-domains.mjs` 会拦）。The bootstrap route
 ...
 */
const REAL_CATALOG = [
    {"id": "example-mcp", "name": "示例 MCP 智能体", "description": "示例远程 MCP 连接器:OAuth 2.1 + PKCE + 授权服务器元数据发现。", "auth_mode": "oauth", "definition": "{\"auth\":{\"discoveryUrl\":\"https://mcp.example.com/mcp\",\"clientId\":\"\",\"authorizeUrl\":\"\",\"tokenUrl\":\"\",\"redirectUri\":\"http://127.0.0.1/callback\",\"pkce\":true,\"publicClient\":true,\"scopes\":\"offline_access\"},\"mcp\":[{\"serverName\":\"example-mcp\",\"transport\":\"streamable-http\",\"url\":\"https://mcp.example.com/mcp\"}]}"},
...
describe('server catalog contract (real definitions)', () => {
  it('parses every enabled definition the server actually ships', () => {
    ...
    expect(defs.map(d => d.id).sort()).toEqual(['example-mcp', 'sales-easy'])
    const exampleMcp = defs.find(d => d.id === 'example-mcp')
    expect(exampleMcp?.authMode).toBe('oauth')
    expect(exampleMcp?.mcp[0]?.transport).toBe('streamable-http')
    expect(exampleMcp?.mcp[0]?.url).toBe('https://mcp.example.com/mcp')
    expect((exampleMcp?.auth as { discoveryUrl?: string })?.discoveryUrl).toBe('https://mcp.example.com/mcp')
    ...
    expect((exampleMcp?.auth as { scopes?: string })?.scopes).toBe('offline_access')
