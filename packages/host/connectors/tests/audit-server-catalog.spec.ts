 ...
 */
const REAL_CATALOG = [
    {"id": "moka", "name": "Moka HR 智能体", "description": "招聘和人事一体的 AI 同事,把查询与执行收进一个对话。人才推荐、招聘动态、考勤绩效、审批待办,一句话问清;智能寻聘、面试分析与面试官评估,一句话发起。", "auth_mode": "oauth", "definition": "{\"auth\":{\"discoveryUrl\":\"https://mcp.example.com/mcp\",\"clientId\":\"\",\"authorizeUrl\":\"\",\"tokenUrl\":\"\",\"redirectUri\":\"http://127.0.0.1/callback\",\"pkce\":true,\"publicClient\":true,\"scopes\":\"offline_access\"},\"mcp\":[{\"serverName\":\"moka\",\"transport\":\"streamable-http\",\"url\":\"https://mcp.example.com/mcp\"}]}"},
...
describe('server catalog contract (real definitions)', () => {
  it('parses every enabled definition the server actually ships', () => {
    ...
    expect(moka?.mcp[0]?.url).toBe('https://mcp.example.com/mcp')
    expect((moka?.auth as { discoveryUrl?: string })?.discoveryUrl).toBe('https://mcp.example.com/mcp')
