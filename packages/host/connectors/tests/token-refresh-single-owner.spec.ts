...
function def(origin: string): ConnectorDef {
  return {
    id: 'moka', name: 'Moka', description: 'single-owner', authMode: 'oauth',
    auth: {
...
describe('SDK 自己续期后，旋转后的 refresh token 必须已经落盘', () => {
  it('saveTokens 返回时磁盘上就是新令牌（不能 fire-and-forget）', async () => {
    ...
    const before = await store.readCredential('moka')
    ...
    const onDisk = await store.readCredential('moka')
...
describe('SDK 的 tokens() 与我们的刷新共用一个单飞（不允许二次出示同一个 refresh token）', () => {
  it('三个并发 tokens() 加上面板刷新 ⇒ 恰好一次续期、零 reuse', async () => {
    ...
    const store = new ConnectorStore({ baseDir: dir })
    const stale = await store.readCredential('moka')
    ...
    const credential = (await store.readCredential('moka'))!
    const target = {
  ...
  it('内置地址与面板刷新并发：我们自己的触发点也不额外续期', async () => {
    ...
    const [, res] = await Promise.all([
      provider!.tokens(),
      callRoute(h, '/api/pico/connectors/moka/refresh', 'POST'),
    ])
