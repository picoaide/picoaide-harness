...
test('S13-3 复核 换入失败且回滚也失败：报可恢复错误、新旧两份副本都还在盘上', () => {
  ...
  assert.match(String(r.entry.message), /\.old-\d+-\d+/, '必须点名旧内容副本路径')
  assert.match(String(r.entry.message), /\.staging-\d+-\d+/, '必须点名新内容副本路径')
...
test('S13-3 复核 崩溃残留：下次同步开头清扫陈旧暂存目录，但不碰并发同步与用户目录', () => {
  ...
  try {
    ...
    mkdirSync(join(userSkills, `${NAME}.staging-${deadPid}-${ancient}`))
    writeFileSync(join(userSkills, `${NAME}.staging-${deadPid}-${ancient}`, 'SKILL.md'), `---\nname: ${NAME}\nx-version: 2\n---\n# GHOST\n`)
    // 2) pid 复用：进程活着但目录早已陈旧
    mkdirSync(join(userSkills, `${NAME}.staging-${process.pid}-${ancient}`))
    // 3) 正在跑的并发同步（活 pid + 新时间戳）→ 不得清扫。
    //    时间戳刻意 +1s：同步自己的暂存目录命名就是 `${pid}-${Date.now()}`，用"此刻"
    //    会和它在同一毫秒内撞名，随后 staging→dest 的 rename 会把这个夹具目录整个搬走
    //    （2026-09-17 三轮复核复现：紧邻调用时 354/400 次）。+1s 不影响语义：
    //    「活 pid + 新时间戳的并发同步目录不得清理」。
    const live = `${NAME}.staging-${process.pid}-${Date.now() + 1000}`
    mkdirSync(join(userSkills, live))
    // 4) 不属于本插件命名空间的目录（技能名不在内置清单）→ 不得清扫
    const foreign = `some-other-skill.staging-${deadPid}-${ancient}`
    ...
    assert.equal(existsSync(join(userSkills, `${NAME}.staging-${deadPid}-${ancient}`)), false, '崩溃残留必须被清扫')
    assert.equal(existsSync(join(userSkills, `${NAME}.staging-${process.pid}-${ancient}`)), false, '超龄残留按陈旧清扫（pid 复用兜底）')
    ...
    assert.deepEqual(readdirSync(userSkills).filter((n) => n.startsWith(`${NAME}.staging-`)), [live], '本插件命名空间内只剩并发同步那一个')
  } finally {
...
test('S13-3 三轮复核：真目录缺失时留下的 .old-* 在本次同步装回后立即回收（不留一整个会话的幽灵技能）', () => {
  ...
  try {
    ...
    const ancient = 1000
    const aside = `${NAME}.old-${deadPid}-${ancient}`
    const staging = `${NAME}.staging-${deadPid}-${ancient}`
    ...
    assert.deepEqual(
      readdirSync(userSkills).filter((n) => n.startsWith(`${NAME}.`)),
      [],
      '技能库里不得再留本插件的换入残留',
    )
  } finally {
