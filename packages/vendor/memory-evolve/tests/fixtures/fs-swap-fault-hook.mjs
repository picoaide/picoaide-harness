 ...
 *
 *   - `<dest>.staging-<pid>-<ts>` → the staging→dest swap (step 2 of the swap);
 *   - `<dest>.old-<pid>-<ts>`     → the aside→dest restore (the rollback path),
 ...
 * the swap. Every other named export is forwarded from the real fs (the export
