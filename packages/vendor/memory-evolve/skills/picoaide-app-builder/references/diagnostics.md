...
| `AI_RATE_LIMITED` | 调用频率 | 减少循环内调用；合并成一次更长的提示词 |
| `AUTH_REQUIRED` | `auth.mode` | 身份相关能力需要登录：把 `access` 改成 `login` / `whitelist`，或引导用户先登录 |
...
- 只在人多的时候失败：先看队列（`APP_QUEUE_FULL` / 429）与单语句耗时，
  不要靠"调大并发"解决 —— 平台没有并发旋钮（每应用串行、队列 32、
  每用户同应用排队 4 个、每用户跨应用在跑 4 个、全局实例 32，全部固定）。
