...
describe('目录渲染（R34 / R36）', () => {
  ...
  it('分享链接只在拿到渠道 scheme 时才产出（未注入 ⇒ null，不回落官方值）', () => {
    ...
    setAppShareScheme('example-a-harness')
    expect(appShareLink('roster')).toBe('example-a-harness://app/roster')
...
describe('打开应用：只有本机路由这一条路（冻结契约 2026-09-19 §4.5）', () => {
  ...
  it('parseAppProtocolURL 只认"本安装的渠道 scheme + 这个 app"（scheme 是运行期值）', () => {
    ...
    expect(parseAppProtocolURL(appURL('demo'), 'demo', 'example-a-harness-app:')).toBeNull()
