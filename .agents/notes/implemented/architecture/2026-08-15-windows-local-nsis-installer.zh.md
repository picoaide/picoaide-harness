...
PicoAide Harness 提供 `yarn dist:win` 作为原生 Windows x64 打包命令。它会先执行 build、全部 TypeScript compiler face、Windows 可运行的打包与原生 shell 聚焦测试，以及 runtime-closure verifier，再让 Electron Builder 为当前版本创建 NSIS 安装向导。POSIX 执行测试仍由跨平台 CI suite 持有，不会阻塞原生 Windows 打包。安装器支持当前用户安装和提升权限后的所有用户安装、可选安装目录，以及开始菜单和桌面快捷方式。稳定的 application ID 仍是 NSIS 的升级身份。
...
Electron Builder 现有的 `afterPack` hook 会在 NSIS 封装前校验 application archive、物理 runtime tree 与必需的 Windows x64 `node-pty` prebuild。打包完成后，第二个 gate 会要求准确版本的安装器与 `win-unpacked/PicoAide Harness.exe` 存在，验证两者都是非空普通文件，并检查其 DOS `MZ` 与 Windows `PE\0\0` signature。其他版本遗留的安装包无法通过该 gate。
