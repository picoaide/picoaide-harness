...
On macOS, LaunchServices opens a generated `welcome.command`. The controlled interactive zsh or bash startup reads the user's ordinary interactive rc file first, then removes Electron Node mode and restores the desktop-owned home and shim path so a user rc cannot accidentally discard them. On Windows, the launcher resolves PowerShell 7, Windows PowerShell, then Command Prompt; an optional Windows Terminal wrapper may host the selected shell. Process creation always uses an executable plus argv with `shell: false`, and the generated PowerShell or batch welcome file performs the same final environment setup.
