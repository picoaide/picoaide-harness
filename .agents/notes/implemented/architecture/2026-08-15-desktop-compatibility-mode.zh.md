...
Package 测试会拒绝导出 `./client` 或声明 `dsh.client` 的兼容 package。Profile 测试验证官方 layout、sidebar 与 conversation row 保持启用。窗口选项测试会拒绝兼容构造器中的高级原生选项；构建后的 Loader smoke 会激活 Host shell 和 profile 本地第三方插件，但不会导入 Electron 或打开窗口。
