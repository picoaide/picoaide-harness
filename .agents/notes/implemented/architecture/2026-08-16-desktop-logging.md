...
The default profile loads `dsh-plugin-desktop/diagnostics`, which contributes an **Export Diagnostics…** native tray command on macOS and Windows. Before creating an archive, `desktopRuntime.exportDiagnostics()` shows a localized privacy confirmation explaining that logs can contain local paths and workspace/session identifiers. Confirmed exports run in a short-lived Node worker so archive reads and compression do not block Electron's main thread. The worker includes at most the newest 50MB of owned regular logs plus a system-info summary, records omitted-file counts, atomically publishes the zip under `userData/diagnostics/`, rejects linked input/output directories, tolerates disappearing or locked files, and retains the three newest archives. The runtime coalesces concurrent requests, reveals a successful archive in the system file manager, and presents a native error dialog on failure.
