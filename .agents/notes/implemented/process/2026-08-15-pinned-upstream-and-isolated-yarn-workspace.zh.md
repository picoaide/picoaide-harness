...
普通桌面构建从 npm registry 解析已发布的 DSH 包，不从子模块链接源码。`upstream.json` 分别记录源码版本和运行时包 family。固定的 GitHub 公开源码声明为 `0.1.0-rc.7`；桌面运行时会继续使用已发布的 `0.1.0-rc.6` family，直到独立的兼容性升级完成验证。当 npm artifact 没有发布对应源码提交时，仓库不会虚构两者的对应关系。
